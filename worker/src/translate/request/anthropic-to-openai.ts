...
export function formatAnthropicToOpenAI(body: any): any {
  ...
  if (tools) {
    // Only client tools carry an input_schema. Anthropic *server* tools
    // (web_search_20250305, web_fetch_20250910, ...) must not be forwarded as
    // function tools — the model would emit a client tool_call nobody executes.
    // Server-side web search is handled separately in the router.
    const clientTools = tools.filter((item: any) => item.input_schema);
    if (clientTools.length > 0) {
      data.tools = clientTools.map((item: any) => ({
        type: "function",
        function: {
          name: item.name,
